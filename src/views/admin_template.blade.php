...
<head>
    ...
    <title>
        {{ isset($page_title) && $page_title
            ? CRUDBooster::getSetting('appname') . ': ' . strip_tags($page_title)
            : CRUDBooster::getSetting('appname') . ': Admin Area' }}
    </title>
...
<body
    class="@php echo (session('theme_color'))?:'skin-blue'; echo ' '; echo config('crudbooster.ADMIN_LAYOUT'); @endphp {{ $sidebar_mode ?: '' }}">
                    ...
                    <div class="section-header">
                        ...
                        @if ($module)
                            <h1>
                                ...
                                <i class="{!! $page_icon ?? null ?: $module->icon ?? 'fa fa-bars' !!}" style="font-size: 18px;"></i> {!! ucwords($page_title ?? null ?: $module->name ?? 'Admin') !!}
                                &nbsp;&nbsp;
                        ...
                        @if (session('message') != '')
                            <div class="alert alert-{{ session('message_type') }} alert-dismissible show fade">
                                <div class="alert-body">
                                    ...
                                    <h5><i class="icon fa fa-info"></i>
                                        {{ cbLang('alert_' . session('message_type')) }}</h5>
                                    {!! session('message') !!}
                                </div>
                            </div>
                        @endif
