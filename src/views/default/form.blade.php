    ...
    <div>
        ...
        <div class="card card-light">
            ...
            <div class="card-body" style="padding:20px 0px 0px 0px">
                <?php
                $row_id = $row->id ?? null;
                $action = $row_id ? CRUDBooster::mainpath("edit-save/$row_id") : CRUDBooster::mainpath('add-save');
                
                $return_url = $return_url ?? null ?: g('return_url');
                ?>
                <form class='form-horizontal' method='post' id="form" enctype="multipart/form-data"
                    action='{{ $action }}'>
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type='hidden' name='return_url' value='{{ $return_url ?? g('return_url') }}' />
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                @if ($button_cancel && CRUDBooster::getCurrentMethod() != 'getDetail')
                                    @if (g('return_url'))
                                        <a href="{{ g('return_url') }}" class="btn btn-outline-secondary">
                                            <i class='fa fa-chevron-circle-left'></i> {{ cbLang('button_back') }}
                                        </a>
                                    @else
