@if (isset($form['datatable']) && $form['datatable'])
    ...
    @else
        @if (isset($form['datatable_ajax']) && $form['datatable_ajax'] == true)
            ...
            $raw = $datatable ? explode(',', $datatable) : [];
...
<div class='form-group {{ $header_group_class }} {{ $errors->first($name) ? 'has-error' : '' }}'
    ...
    <div class="{{ $col_width ?: 'col-sm-10' }}">
        <select style='width:100%' class='form-control' id="{{ $name }}" {{ $required }} {{ $readonly }}
            ...
            name="{{ $name }}{{ $form['relationship_table'] ?? null ? '[]' : '' }}"
            {{ $form['relationship_table'] ?? null ? 'multiple="multiple"' : '' }}>
            ...
            @if (isset($form['datatable']) && $form['datatable'])
                @if (isset($form['relationship_table']) && $form['relationship_table'])
                    ...
                    if (isset($form['datatable_orig']) && $form['datatable_orig'] != '') {
                        ...
                        $current_id = isset($row) && isset($row->id) ? $row->id : null;
                    
                        $row_orig = DB::table($params[0])->where($params[2], $current_id)->first();
                        $value = $row_orig ? explode(',', $row_orig->{$params[1]}) : [];
                    } else {
                        $current_id = isset($row) && isset($row->id) ? $row->id : null;
                        ...
                        if ($current_id) {
                            ...
                            $value = DB::table($form['relationship_table'])->where($foreignKey, $current_id)->pluck($foreignKey2)->toArray();
                        } else {
                            $value = [];
                        }
                    }
