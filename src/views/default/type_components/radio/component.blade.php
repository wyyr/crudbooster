<div class='form-group {{ $header_group_class }} {{ $errors->first($name) ? 'has-error' : '' }}'
    ...
    <div class="{{ $col_width ?: 'col-sm-10' }}">
        ...
        @if (!($form['dataenum'] ?? null) && !($form['datatable'] ?? null) && !($form['dataquery'] ?? null))
        ...
        @if (isset($form['dataenum']) && $form['dataenum'] != '')
            ...
            if (!is_array($current_value)) {
                $current_value = explode(';', (string) $current_value);
            }
            array_walk($current_value, 'trim');
            ...
            @foreach ($dataenum as $k => $d)
                ...
                $checked = ($current_value && in_array($val, $current_value)) || (CRUDBooster::isCreate() && ($k == 0 && ($form['validation'] ?? null))) ? 'checked' : '';
                ?>
                <div class=" {{ $disabled }}">
                    <label class='radio-inline'>
                        <input type="radio" {{ $disabled }} {{ $checked }} name="{{ $name }}"
                            value="{{ $val }}"> {{ $label }}
                    </label>
                </div>
            @endforeach
        ...
        if (@$form['datatable']):
            ...
            foreach ($selects_data as $d) {
                ...
                echo "<div data-val='$val' class='input-radio-wrapper $disabled'><label class='radio-inline'><input type='radio' $disabled $checked name='" . $name . "' value='" . $d->id . "'> " . $val . '</label></div>';
            }
        endif;
        if (isset($form['dataquery']) && $form['dataquery']) {
            ...
            if ($query) {
                foreach ($query as $q) {
                    ...
                    $data_val = $q->label ?? '';
                    echo "<div data-val='$data_val' class=' $disabled'><label class='radio-inline'><input type='radio' $disabled $checked name='" . $name . "' value='$q->value'> " . $q->label . '</label></div>';
                }
            }
        }
        ?>
