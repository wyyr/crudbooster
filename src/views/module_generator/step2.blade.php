...
@section('content')
    ...
    <ul class="nav nav-tabs nav-primary mb-1">
        ...
        <li class="nav-item" role="presentation">
            <a class="nav-link active" href="{{ Route('ModulControllerGetStep2') . '/' . $id }}">
    ...
    <form method="post" action="{{ Route('ModulControllerPostStep3') }}">
        <div class="card card-light">
            <div class="card-header">
                <h6 class="card-title">Table Display</h6>
            </div>
            <div class="card-body">
                <div class="alert alert-info">
                    <strong>Warning</strong>. Make sure that your column format are normally, unless using this Tool maybe
                    make your current configuration broken,
                ...
                <table class="table-display table table-striped table-sm">
                    <thead>
                        <tr>
                            <th>Column</th>
                            <th>Name</th>
                    ...
                    <tbody>
                            ...
                            @foreach ($cb_col as $c)
                                <tr>
                                    ...
                                    <td><input value='{{ @explode(',', $c['join'])[0] }}' type='text'
                                            name='join_table[]' onclick='showTable(this)' onKeyUp='showTableLike(this)'
                                            placeholder='Table Name' class='join_table form-control notfocus'
                                            value='' /></td>
                                    ...
                                    <td><input type='text' name='callbackphp[]' class='form-control callbackphp notfocus'
                                            value='{{ $c['callback_php'] ?? '' }}' placeholder="Optional" /></td>
                                    <td><input value='{{ $c['width'] ?? 0 }}' type="number" name="width[]"
                                            class="form-control pr-0" /></td>
                                    <td>
                                        <select class="form-control is_image pr-0" name="is_image[]">
                                            <option {{ !($c['image'] ?? 0) ? 'selected' : '' }} value='0'>N</option>
                                            <option {{ $c['image'] ?? 0 ? 'selected' : '' }} value='1'>Y</option>
                                        </select>
                                    </td>
                                    <td>
                                        <select class="form-control is_download" name="is_download[]">
                                            <option {{ !($c['download'] ?? 0) ? 'selected' : '' }} value='0'>N
                                            </option>
                                            <option {{ $c['download'] ?? 0 ? 'selected' : '' }} value='1'>Y</option>
                                        </select>
                                    </td>
                                    <td>
                        ...
                        <tr id="tr-sample" style="display:none">
                            ...
                            <td><input type='text' name='join_table[]' onclick='showTable(this)'
                                    onKeyUp='showTableLike(this)' placeholder='Table Name'
                                    class='join_table form-control notfocus' value='' /></td>
                            <td><input type='text' name='join_field[]' onclick='showTableField(this)'
                                    onKeyUp='showTableFieldLike(this)' placeholder='Field Name Shown'
                                    class='join_field form-control notfocus' value='' /></td>
                            <td><input type='text' name='callbackphp[]' class='form-control callbackphp notfocus'
                                    value='' placeholder="Optional" /></td>
                            <td><input type='number' name='width[]' value='0' class='form-control pr-0' /></td>
                            <td>
                                <select class="form-control is_image pr-0" name="is_image[]">
                            ...
                            <td>
                                <select class="form-control is_download pr-0" name="is_download[]">
                                    <option value="0">N</option>
                                    <option value="1">Y</option>
                                </select>
                            </td>
