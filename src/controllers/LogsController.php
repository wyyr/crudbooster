...
class LogsController extends CBController
{
    ...
    public static function displayDiff($old_values, $new_values)
    {
        ...
        foreach ($diff as $key => $value) {
            ...
            $table .= '<tr>
                    ...
                    <td>'.(is_array($old) ? json_encode($old) : $old).'</td>
                    <td>'.(is_array($new) ? json_encode($new) : $new).'</td>
                  </tr>';
        }
    ...
    private static function getDiff($old_values, $new_values)
    {
        ...
        $unset_keys = ['id', 'created_at', 'updated_at', 'deleted_at'];

        foreach ($unset_keys as $k) {
            unset($old_values[$k], $new_values[$k]);
        }

        return array_diff_assoc($new_values, $old_values);
    }
}
