...
class MenusController extends CBController
{
    public function cbInit()
    {
        ...
        $row = (Request::segment(3) == 'edit') ? CRUDBooster::first($this->table, $id) : null;
        $row_type = ($row) ? $row->type : 'Module';
        $row_path = $row->path ?? '';
        ...
        if ($row_type == 'Module') {
            ...
            $clean_path = str_replace('statistic_builder/show/', '', $row_path);
            $m = CRUDBooster::first('cms_statistics', ['slug' => $clean_path]);
            $id_statistic = $m->id ?? 0;
        }
        ...
        $this->form[] = [
            ...
            'value' => $id_module,
            'style' => ($row_type !== 'Module') ? 'display:none' : '',
            'dataenum' => null,
        ...
        $this->form[] = [
            ...
            'value' => $id_statistic,
            'style' => ($row_type !== 'Statistic') ? 'display:none' : '',
        ];
        ...
        $this->form[] = [
            ...
            'style' => (in_array($row_type, ['Module', 'Statistic'])) ? 'display:none' : '',
        ];
        ...
        $id_cms_privileges = Request::get('id_cms_privileges') ?: CRUDBooster::myPrivilegeId();
    ...
    public function getIndex()
    {
        ...
        $all_menus = DB::table('cms_menus')
            ->orderby('sorting', 'asc')
            ->get();

        $menu_active = $all_menus->where('parent_id', 0)->where('is_active', 1);
        $menu_inactive = $all_menus->where('parent_id', 0)->where('is_active', 0);

        foreach ($all_menus as $menu) {
            $menu->children = $all_menus->where('parent_id', $menu->id)->values();
        }
